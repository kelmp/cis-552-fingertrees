{-# LANGUAGE TypeSynonymInstances #-}
{-# LANGUAGE FlexibleInstances #-}

module Main where

import Prelude hiding (tail, fromList)
import FingerTree as FT (FingerTree (..), tail, fromList, (!!), insertHead,
  insertTail, Measured (..))
import System.Random (randomRIO)
import Text.Read (readMaybe)
import Data.Maybe (fromMaybe, fromJust)

data Person = Person
  { name :: String,
    adj :: String,
    memberFor :: String,
    fastPass :: Bool }

instance Measured String where
  measure s = 1

instance Measured Person where
  measure p = 1

durTree :: FingerTree String
durTree = fromList ["seconds", "minutes", "hours", "days", "weeks", "months",
  "years", "decades", "centuries", "millenia"]

nameTree :: FingerTree String
nameTree = fromList ["Alice", "Bob", "Carol", "Dave", "Eve", "Frank",
  "Grace", "Heidi", "Ivan", "Judy", "Mallory", "Oscar", "Peggy", "Rupert",
  "Sybil", "Ted", "Victor", "Walter", "Wendy", "Adrian", "Yoni"]

adjTree :: FingerTree String
adjTree = fromList ["loyal", "satisfied", "happy", "star",
  "aggressively nice", "friendly"]

newPerson :: IO Person
newPerson = do
  nameI <- randomRIO (0, length nameTree - 1)
  adjI <- randomRIO (0, length adjTree - 1)
  durI <- randomRIO (0, length durTree - 1)
  time <- randomRIO (2, 9)
  return $ fromJust $ newFromIndices nameI adjI durI time where
    newFromIndices :: Int -> Int -> Int -> Int -> Maybe Person
    newFromIndices n a d t = do
      name <- nameTree FT.!! n
      adj <- adjTree FT.!! a
      dur <- durTree FT.!! d
      return $ Person { name = name,
        adj = adj,
        memberFor = show t ++ " " ++ show d,
        fastPass = n `mod` 3 == 0}

addPerson :: FingerTree Person -> IO (FingerTree Person)
addPerson ft = do
  p <- newPerson
  if fastPass p then return $ insertHead p ft
    else return $ insertTail p ft

init :: Int -> IO (FingerTree Person)
init n = enq n Nil

bio :: Person -> String
bio p = name p ++ " has been a " ++ adj p ++ " customer of Not-Disney for "
  ++ memberFor p

complain :: Person -> String
complain p = bio p ++ ", but that's going to change if they can't speak \
  \to a manager RIGHT NOW."

getInt :: IO Int
getInt = do
  input <- getLine
  case readMaybe input of
    Nothing -> do
      putStrLn "Please enter a valid integer."
      getInt
    Just x -> return x

deq :: Int -> FingerTree Person -> FingerTree Person
deq 0 ft = ft
deq n ft = case ft of
  Nil -> Nil
  _ -> deq (n - 1) (tail ft)

enq :: Int -> FingerTree Person -> IO (FingerTree Person)
enq 0 ft = return ft
enq n ft = do
  ft' <- addPerson ft
  enq (n - 1) ft'

queueLoop :: FingerTree Person -> IO ()
queueLoop q = let len = length q in do
  putStrLn $ "There are currently " ++ show len ++ " people in the queue."
  angryI <- randomRIO (0, length q - 1)
  let angry = fromJust $ q FT.!! angryI in do
    putStrLn $ complain angry
    putStrLn "Please type your response to this well-behaved customer."
    input <- getLine
    putStrLn "After hearing your soothing words, the customer heads back to \
    \their spot in line."
    putStrLn "How many people would you like to let into the ride?"
    admitted <- getInt
    toAdd <- randomRIO (1, 10)
    queue' <- enq toAdd $ deq admitted q
    queueLoop queue'

main :: IO ()
main = do
  putStrLn "Welcome to Not-Disney Not-Land-Or-World! How many people have \
  \rushed into the renowned Cosmos-Themed Landform Ride as soon as it opened?"
  qSize <- getInt
  queue <- Main.init qSize
  queueLoop queue
